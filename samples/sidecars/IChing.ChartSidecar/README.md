...
| POST | `/bazi` | 请求体 `{ "args": { ... } }`，返回 `{ engine, chart }` |
| POST | `/liuyao` | 请求体 `{ "args": { "method", "at", "seed" } }`，返回 `{ engine, chart }` |
| POST | `/tarot` | 请求体 `{ "args": { "spreadId", "question", "seed" } }`，返回 `{ engine, reading }`（Deckaura enrich） |
| POST | `/calendar` | 请求体 `{ "args": { "year", "month", "day", "sect" } }`，返回 `{ engine, day }` |
