...
public partial class FollowUpChatPage : ContentPage
{
    ...
    private void UpdateRoundChrome()
    {
        RoundLabel.Text = $"第 {_rounds}/3 轮";
        var atLimit = _rounds >= 3;
        LimitHintLabel.IsVisible = atLimit;
        QuestionEntry.IsEnabled = !atLimit;
        SendButton.IsEnabled = !atLimit;
        if (atLimit)
        {
            QuestionEntry.Placeholder = "已达追问上限";
        }
    }
    ...
    private async void OnSendClicked(object? sender, EventArgs e)
    {
        ...
        var followUp = App.Interpretation.ShouldUseLabFollowUp(labSessionId)
            ? App.Interpretation.StreamLabFollowUpAsync(labSessionId!, text)
            : App.Interpretation.StreamFollowUpAsync(messages);
        ...
        // Lab 路径已由 /lab/chat followup 落库；BYOK 路径才需 AppendLabHistory 同步。
