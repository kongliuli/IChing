...
$dst = Join-Path $root "src/IChing.Tarot.App/Resources/Images/tarot"
...
$count = 0
foreach ($f in $files) {
    Copy-Item $f.FullName (Join-Path $dst "tarot_$($f.Name)") -Force
    $count++
}
Write-Host "已同步 $count 张牌面图到 MAUI Resources/Images/tarot/"
